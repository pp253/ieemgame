...
export function setDeliver(req, res, next) {
  ...
  req.getValidationResult().then(function(result) {
    ...
    if (team.selectJob(job).storage.getStorage(product) < amount) {
      ...
      res.json(
        response.ResponseErrorMsg.StorageNotEnough(gameId, teamIndex, job)
      )
      return
    }
    ...
    if (
      job !== 'RETAILER' &&
      team.selectJob(job).order.getOrder(product) <
        team.selectJob(job).deliver.getDeliver(product) + amount
    ) {
      ...
      job === 'RETAILER' &&
      game.getMarket().orderAmount - game.getMarket().storageAmount < amount
    ) {
