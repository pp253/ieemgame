...
export default class Game {
  ...
  nextGameStage() {
    let list = [
      constant.GAME_STAGE.UNKNOWN,
      constant.GAME_STAGE.PREPARE,
      constant.GAME_STAGE.READY,
      constant.GAME_STAGE.START,
      constant.GAME_STAGE.FINAL,
      constant.GAME_STAGE.END
    ]

    if (this.getGameStage() === constant.GAME_STAGE.END) {
      return this
    }
    ...
    debug.log(
      `GameId='${this.getGameId()}' Stage has been set to ${this.getGameStage()}`
    )
    return this
  }
  ...
  nextDay() {
    if (this.isOffWork()) {
      ...
      debug.log(
        `GameId='${this.getGameId()}' day has been set to ${this.getDay()}`
      )
    }
    return this
  }
  ...
  isWorking() {
    return (
      this.getGameStage() === constant.GAME_STAGE.START &&
      this.getDayStartTime() !== constant.UNKNOWN_TIME
    )
  }
  ...
  _update() {
    switch (this.getGameStage()) {
      ...
      case constant.GAME_STAGE.START:
        ...
        // check news
        this.getMarket().orderAmount = this.getNews().getDemanded(
          this.getDay(),
          this.getTime()
        )
        this.getMarket().price = this.getNews().getPrice(
          this.getDay(),
          this.getTime()
        )
        break
  ...
  settle() {
    ...
    for (let team of this.getTeamList()) {
      ...
      team.getAccount().take(
        constant.AccountItem({
          day: this.getDay(),
          time: this.getTime(),
          balance: cost
        })
